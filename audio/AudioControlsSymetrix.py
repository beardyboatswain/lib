...
from lib.var.lib_debug_mode import AudioControlsSymetrix_dbg
dbg = debugger(AudioControlsSymetrix_dbg, __name__)


class SymetricsConverter(object):
    def __init__(self, max: int = 12, min: int = -72):
        self.max = max
        self.min = min

    def itodb(self, symetricsValue: int) -> int:
        """
        Converts symetrix value to db value
        """
        return round(self.min + (self.max - self.min) * (int(symetricsValue) / 65535))

    def itosv(self, dbValue: int) -> int:
        """
        Converts db value to symetrix value
        """
        return int((int(dbValue) - self.min) * 65535 / (self.max - self.min) + 0.5)

    def itobool(self, symetricsValue: int) -> bool:
        """
        Converts symetrix value to db value
        """
        return False if (int(symetricsValue) == 0) else True

    def booltosv(self, stValue: bool) -> int:
        """
        Converts db value to symetrix value
        """
        return 65535 if stValue else 0


class FaderControl():
    def __init__(self,
                 dev: AudioProxySymetrix,
                 levelRcId: int,
                 muteRcId: int,
                 name="Level",
                 comment="",
        ...
        # self.ui = ui
        self.dev = dev
        self.levelRcId = levelRcId
        self.muteRcId = muteRcId
        self.name = name
        ...
        self.sc = SymetricsConverter(self.max, self.min)
        ...
        self.dev.addLevel(levelRcId=self.levelRcId, muteRcId=self.muteRcId, callbackMethod=self.fbHandler)

    def __showlvl__(self):
        # dbg.print('showlvl new level {} = {}'.format(self.levelRcId, self.channel, self.value))
    ...
    def __inc__(self):
        if (self.value + self.step <= self.max):
            ...
            self.value = self.max
        self.__showlvl__()
        self.dev.setLevel(levelRcId=self.levelRcId, value=self.sc.itosv(self.value))
    ...
    def __mute__(self):
        ...
        self.__showmute__()
        self.dev.setMute(muteRcId=self.muteRcId, value=self.sc.booltosv(self.mute))
    ...
    def fbHandler(self, rcId, value):
        if (rcId == self.levelRcId):
            self.value = self.sc.itodb(value)
            self.__showlvl__()
        elif (rcId == self.muteRcId):
            dbg.print("Mute {} recieve value {} ({})".format(rcId, value, self.sc.itobool(value)))
            self.mute = self.sc.itobool(value)
            self.__showmute__()
...
class SignalProbe():
    def __init__(self,
        ...
        self.btnProbe = None
        if ((self.btnId > 0) and self.uiHost):
            self.btnProbe = Button(self.uiHost, self.btnId)
    ...
    def setMechanics(self, btnProbeId: int):
        self.btnProbe = Button(self.uiHost, btnProbeId)
        self.__showSignal__()
...
class PresetControl():
    def __init__(self, dev: AudioProxySymetrix):
        self.dev = dev
...
class CrosspointControl():
    def __init__(self,
        ...
        self.btnCrosspoint = list()

        self.states = dict.fromkeys(self.rcIds)
        self.value = False

        for cpId in self.rcIds:
            self.dev.addCposspoint(cpId, self.fbHandler)
    ...
    def setMechanics(self, ui: UIDevice, btnId: int):
        newBtn = Button(ui, btnId)
        newBtn.SetText(self.name)

        self.btnCrosspoint.append(newBtn)

        @event(self.btnCrosspoint, sStates)
...
class MuteControl():
    ...
    def fbHandler(self, itType: str, value: str):
        if (itType == AudioProxySymetrix.tagMute):
            self.mute = (value == "On")
            self.__showmute__()
