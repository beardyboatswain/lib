...
class PseudoSeamless(VideoControlProxyMeta):
    ...
    def add_seamless_matrix(self, matrix: VideoControlProxyMeta, out_size: int) -> None:
        self.sm_matrix = matrix
        self.sm_out_size = out_size
        for i_out in range(1, out_size + 1):
            self.sm_states[i_out] = 0
        for i_out in range(1, out_size + 1):
            self.states[i_out] = matrix.getTie(i_out)

    def add_break_video_matrix(self, matrix: VideoControlProxyMeta, in_size: int, outs: List[int]) -> None:
        self.bv_matrix = matrix
        self.bv_in_size = in_size
        self.bv_outs = outs
        for i_out in outs:
            self.bv_states[i_out] = matrix.getTie(i_out)

    def add_wiring(self, wiring: Dict[int, int]) -> None:
        '''
        wiring: Dict[int, int] = {<seamless_matrix_input>: <connected_to_break_video_matrix_output>}
        '''
        self.wiring = wiring

    def update_states(self) -> None:
        for i_out in range(1, self.sm_out_size + 1):
            self.states[i_out] = self.sm_matrix.getTie(i_out)

    def setTie(self, nOut: int, nIn: int):
        if nIn not in self.bv_states.values():
