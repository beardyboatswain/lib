...
class LcdProxySamsHG(object):
    def __init__(self, dev, rfbPowerCallback=None):
        ...
        self.dbg = debugger.debugger(LcdProxySamsHG_dbg, self.__class__)
    ...
    def power(self, nPower):
        ...
        if (nPower.title() == "On") or ((nPower.title() == "Tgl") and (self._power == "Off")):
            self._dev.send(b'\x68\x80\x00\x01\x80\x69')
        elif (nPower.title() == "Off") or ((nPower.title() == "Tgl") and (self._power == "On")):
...
class LcdProxySamsDM(object):
    def __init__(self, dev, rfbPowerCallback=None):
        ...
        self.dbg = debugger.debugger(LcdProxySamsDM_dbg, self.__class__)
...
class LcdGSProxy(object):
    def __init__(self, dev, rfbPowerCallback=None):
        ...
        self.dbg = debugger.debugger(LcdGSProxy_dbg, self.__class__)
        ...
        def lcdStatusHandler(command, value, qualifier):
            self.dbg.print('LCD Cmd={} Val={} Qaul={}'.format(command, value, qualifier))
            if (command == 'Power'):
                self._power = value
    ...
    def power(self, nPower):
        ...
        self.dbg.print("nPower.title() is On = {}".format((nPower.title() is "On")))
...
class ProjectorGSProxy(object):
    def __init__(self, dev, rfbPowerCallback=None, rfbLampUsageCallback=None):
        ...
        self.dbg = debugger.debugger(ProjectorGSProxy_dbg, self.__class__)
    ...
    def projStatusHandler(self, command, value, qualifier):
        self.dbg.print('Projector Cmd={} Val={} Qaul={}'.format(command, value, qualifier))
        if (command == 'Power'):
