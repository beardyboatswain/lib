...
class MultiDevicePower(DevicePowerMeta):
    ...
    def addDevice(self, *devs: DevicePowerMeta) -> None:
        for iDev in devs:
            if iDev:
                iDev.add_fb_callback_function(self._fbHandler)
                self.devs.append(iDev)
    ...
    def addPowerTglBtn(self, uiHost: UIDevice, btnTglPowerId: int) -> None:
...
class MainPower(object):
    def __init__(
        ...
        self.msg = {
            "On": "Зал включен",
            "Off": "Зал выключен",
            "Warming Up": "Зал включается",
            "Cooling Down": "Зал отключается",
        }
    ...
    def powerOn(self):
        ...
        signals.emit(
            "*",
            signal="roomkit",
            params={"cmd": "standby", "action": "set", "state": "Deactivate"},
        )
    ...
    def rfb(self, power):
        ...
        # переходить на страницу включенного зала, если все девайсы включены
            ...
            dbg.print(
                "Power IS ON. onDev={} self._power={}".format(onDevs, self._power)
            )
        # переходить на страницу отключенного зала, если все девайсы выключены
            ...
            self._ShowPage(self.powerOffPageName)
            dbg.print(
                "Power IS OFF. offDev={} self._power={}".format(offDevs, self._power)
            )
...
class DevicePower(object):
    def __init__(
        ...
        self.msg = {
            "On": "Включено",
            "Off": "Отключено",
            "Warming Up": "Включается",
            "Cooling Down": "Отключается",
        }
...
class DevicePowerMultiBtn(object):
...
class MainPowerMultiButton(object):
    ...
    def setMechanic(
        ...
        if (powerOffPageName is not None) or (powerOnPageName is not None):
            self._Pages.append(
                {
                    "UIHost": UIHost,
                    "OnPage": powerOnPageName,
                    "OffPage": powerOffPageName,
                }
            )
    ...
    def powerOff(self):
        ...
        if self._devName == "Питание зала":
            ...
            signals.emit(
                "*",
                signal="roomkit",
                params={"cmd": "standby", "action": "set", "state": "Activate"},
            )
