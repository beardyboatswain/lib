...
class CameraControlProcessor():
    ...
    def setActiveCamera(self, camID: int) -> None:
        # dbg.print('setActiveCamera - new camID: {}'.format(camID))
        # dbg.print('setActiveCamera - new camIN: {}'.format(self.camInputs[camID]))
        if self.activeCam:
            if camID != self.activeCam.id:
    ...
    def getCameraBySwitchInput(self, input: int) -> CameraControlMeta:
        for iCamID in self.camInputs.keys():
...
class CameraControlPanel(object):
    ...
    def addCameraSelector(self, firstCamBtnId: int) -> None:
        ...
        def camSelectBtnsEventHandler(btn: Button, state: str):
            if state == sReleased:
                dbg.print('sel btn pressed - btnID: {}. btn camID: {}'.format(btn.ID, getattr(btn, "camID")))
    ...
    def fbCamActive(self, cam: CameraControlMeta):
        self.activeCam = cam
        if self.activeCam and (len(self.camSelectBtns) > 0):
